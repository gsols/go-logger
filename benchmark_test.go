...
func BenchmarkLogEmpty(b *testing.B) {
	instance := logger.New(io.Discard)
	b.ResetTimer()
...
func BenchmarkDisabled(b *testing.B) {
	instance := logger.New(io.Discard).Level(zerolog.Disabled)
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			instance.Info().Msg(fakeMessage)
		}
	})
}
...
func BenchmarkContextFields(b *testing.B) {
	instance := logger.New(io.Discard).With().
		Str("string", "four!").
		Time("time", time.Time{}).
		Int("int", 123).
...
func BenchmarkLogArrayObject(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		arr.Object(&obj2)
		arr.Object(&obj3)
		instance.Info().Array("objects", arr).Msg("test")
	}
}
...
func BenchmarkLogFieldType(b *testing.B) {
	...
	types := map[string]func(e *zerolog.Event) *zerolog.Event{
		...
		"Ctx": func(e *zerolog.Event) *zerolog.Event {
			return e.Ctx(ctx)
		},
